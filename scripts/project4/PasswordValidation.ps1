...
function Send-StageStatusEmail{
    ...
    if(-not (test-path $ReportPath)){
        $MailMessage += '<p>Hello Team,<br><br>Your initiated, Password Validation report, has been completed without final report. Please contact your administrator.</br></br></p>'
    }
    else{
        $MailMessage += '<p>Hello Team,<br><br>Your initiated, Password Validation report, has been completed. Please check the logs below and take appropriate action.</br></br></p>'
...
$PasswordValidationReportFile = Join-Path $PSScriptRoot "PasswordValidationReport.xlsx"
Start-Log -LogFileName "log_PasswordValidation.csv"
Archive-Report $PasswordValidationReportFile
...
try{
    ...
    foreach($server in $VcenterServers){
        try{
            ...
            $PasswordValidationReport += '' | Select @{n="VCenterServer";e={$server}},@{n="Status";e={"success"}}  
        }
        catch{
            $PasswordValidationReport += '' | Select @{n="VCenterServer";e={$server}},@{n="Status";e={"fail"}}
            Log-Error -Message "Failed to validate password for VIServer."
    ...
    if($PasswordValidationReport)
    {
        Log-Info -Message "Generating consolidated report."
        $PasswordValidationReport | Export-Excel $PasswordValidationReportFile -WorksheetName "PasswordValidationReport" 
    }
}
catch{
    Log-Error -Message $("ErrorRecord : {0}, CommandName: {1}, Message: {2}" -f $_.Exception.ErrorRecord, $_.Exception.CommandName, $_.Exception.Message)
    ...
    Send-StageStatusEmail -FromEmail $FromEmail -ToEmail $($ToEmail -split ",") -Subject "Password Validation Report." -ReportPath $PasswordValidationReportFile
}
